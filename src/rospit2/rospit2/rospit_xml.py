...
class SubscriberFactory(object):
    ...
    def parse_message_elem(self, elem):
        assert(elem.tag == with_ns("Message") or elem.tag == with_ns("MessageYaml"))
...
class Parser(object):
    ...
    def parse(self):
        self.test_suite = ROSTestSuite(self.node, self.subs, self.root.attrib['name'])
    ...
    def get_test_case_from_xml_element(self, element):
        ...
        test_case = ROSDeclarativeTestCase(self.test_suite, name=element.attrib['name'], wait_for_preconditions=wait_for_preconditions, depends_on=depends_on)
        ...
        for child in element.getchildren():
            ...
            if name == with_ns("SetUp"):
                ...
                for invariant in child.getchildren():
                    test_case.invariants.append(self.get_condition_from_xml_element(invariant))
    ...
    def get_parameters(self, element):
        if element.tag == with_ns("Message"):
            ...
            raise Exception("Unidenfied message specification {}".format(element.tag))
    ...
    def get_step_from_xml_element(self, element):
        ...
        if elem_type == "Dummy":
            ...
            return ServiceCall(self.node, element.attrib['service'], element.attrib['type'], parameters, save_result=save_result)
            ...
            if len(element) > 0:
                ...
                parameters = dict()
            return Publish(self.node, topic, msg_type, duration, rate, parameters)
    ...
    def evaluator_factory(self, element):
        ...
        if elem_type == "StaticBooleanEvaluator":
            ...
            return MessageReceivedEvaluator(self.node, element.attrib['topic'], element.attrib['type'], element.attrib.get('field', None))
        elif elem_type == "MessageEvaluator":
            return MessageEvaluator(self.node, element.attrib['topic'], element.attrib['type'], element.attrib.get('field', None))
            ...
            return NumericMessageEvaluator(self.node, element.attrib['topic'], element.attrib['type'], element.attrib.get('field', None))
        else:
            raise ValueError("Unexpected type {}".format(elem_type))
